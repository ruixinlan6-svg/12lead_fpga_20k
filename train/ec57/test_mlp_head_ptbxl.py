...
m = TinyECGCNN_NV_MLP()
print(f"TinyECGCNN_NV_MLP total parameters: {count_params(m)} (Budget: <= 2048)")
...
class PTBXLBeatDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_training:
            ...
            wave = wave * gain
            # Gaussian noise
            snr_db = np.random.uniform(15.0, 30.0)
...
class_weight = torch.tensor([1.0, 12.0], dtype=torch.float32).to(device)
criterion = nn.CrossEntropyLoss(weight=class_weight)
optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=50, eta_min=1e-5)

print("Starting training with MLP fusion head on GPU 2...")
...
print("\nDetailed Threshold Sweep on Test Split (MLP Head):")
