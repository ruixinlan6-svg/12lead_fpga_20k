...
- `train/` 已包含 PTB-XL 登记、TinyECGCNN FP32 smoke 和静态 INT8 PTQ 参考脚本；数据下载、远端训练、JTAG 扫描、综合、下载和烧录仍必须由明确任务授权，不要仅因阅读本计划而自动执行。
...
- 用户已确认当前实物为 QN88；现有工程的 QN88P/PSRAM 配置只作历史参考。任何 NPU/存储架构开始前仍必须完成非破坏性 SDRAM 读写测试；未通过前不得宣称板级闭环或写入 Flash。
...
- 远程任务无进度、异常退出或重复失败时，不得只做轮询等待；应立即检查进程、日志、输入清单和资源状态，采用可追溯的重试/替代路径，仍无法推进时给出明确的人工接入点。
