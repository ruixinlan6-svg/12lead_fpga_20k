...
module tb_scheme_b_top_27mhz;
    ...
    always #18.518 clk = ~clk; // 27 MHz clock period = 37.037 ns

    localparam integer CPB = 234;
    ...
    initial begin
        ...
        $display("[SIM-27MHz] 1. Sending ECG0 Header...");
        send_byte("E");
        send_byte("C");
        send_byte("G");
        send_byte("0");

        $display("[SIM-27MHz] 2. Sending 12,000 Input Waveform bytes...");
        send_file_bytes("runs/20260826-1929-m2-input-quant-contract/hex/input.hex", 12000);

        $display("[SIM-27MHz] 3. Sending 10,293 Parameter bytes...");
        ...
        $display("[SIM-27MHz] All bytes sent! Waiting for top-level inference to complete...");
        @(posedge dut.model_done);
        $display("[SIM-27MHz] Model Done! Checking Output Logits:");
        ...
        if (dut.out_l0 == 32 && dut.out_l1 == -22 && dut.out_l2 == -21 && dut.out_l3 == -19 && dut.out_l4 == -21) begin
            $display(">>> [PASS] 27MHz Top-Level Simulation Matches Golden Perfectly! <<<");
        end else begin
            $display(">>> [FAIL] Logit Mismatch in 27MHz Top-Level Simulation! <<<");
        end
        $finish;
    end
