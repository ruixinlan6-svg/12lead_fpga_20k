...
print(f"2. Sending {len(input_bytes)} input bytes in 50-byte chunks...")
for off in range(0, len(input_bytes), 50):
    ser.write(input_bytes[off : off + 50])
    ser.flush()
    time.sleep(0.002)

print(f"3. Sending {len(weight_bytes)} weight bytes in 50-byte chunks...")
for off in range(0, len(weight_bytes), 50):
    ser.write(weight_bytes[off : off + 50])
    ser.flush()
    time.sleep(0.002)

print("All bytes transmitted! Draining old RX and waiting for fresh D1 frame...")
time.sleep(0.15) # Wait for core inference (takes ~25 ms)

# Now read the newest line
ser.reset_input_buffer()
time.sleep(0.15)
...
while time.monotonic() < deadline:
    raw = ser.readline()
    if raw:
        line_str = raw.decode("ascii", "replace").strip()
        if line_str.startswith("ECG ") and " D1 " in line_str:
            print(f"  FRESH BOARD RX: {line_str}")
...
ser.close()
print("Timeout waiting for fresh D1 frame!")