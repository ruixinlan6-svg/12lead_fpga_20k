...
def forward(inp):
    # L1: Conv1D (12->16, K=7, pad=3) + ReLU1 + MaxPool1
    # inp shape: 12 x 1000
            ...
            for sub in range(2):
                ...
                for ic in range(12):
                    for k in range(7):
                        t_k = t_in + k - 3
                        if 0 <= t_k < 1000:
                            w_val = w1[oc * 84 + ic * 7 + k]
                            in_val = inp[ic * 1000 + t_k]
                            acc += in_val * w_val
    ...
    # L2: Conv1D (16->32, K=7, pad=3) + ReLU2 + MaxPool2
    ...
    # L3: Conv1D (32->32, K=5, pad=2) + ReLU3
...
print("Normal forward logits:", forward(input_data))
print("Zero input forward logits:", forward(np.zeros(12000, dtype=np.int64)))